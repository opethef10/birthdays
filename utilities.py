"""Utility functions that are useful to make date and calendar calculations"""
...
def doğumGünleri(name: str, birthdate: date, today: date) -> str:
    ...
    return f"{name:<18} {birthdate:%d/%m/%Y %a} {days:>6} days {weeks:>5} weeks {current_age:>6.2f} years"
...
def fixedCal(today: date) -> str:
    ...
    if day <= 0:
        # In case the divmod or leap year calculation makes the day 0 or less
        month -= 1
        day += 28
